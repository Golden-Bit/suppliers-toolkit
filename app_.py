...
pages = {
    ...
    "Report Fornitori": "supplier_reports",
    ...
    if st.button("Visualizza Fornitore", use_container_width=True) or "last_selected_supplier" in st.session_state and st.session_state.last_selected_supplier == selected_supplier_id:
        ...
        st.markdown(f"""
            <div style="text-align: center;">
                ...
                <input type="range" id="zoom" name="zoom" min="0.5" max="2" step="0.1" value="1" oninput="document.getElementById('preview-iframe').style.transform = 'scale(' + this.value + ')'; document.getElementById('preview-iframe').style.transformOrigin = '0 0';">
            </div>
            <script>
                document.getElementById('zoom').addEventListener('input', function() {{
                    var iframe = document.getElementById('preview-iframe');
                    iframe.style.transform = 'scale(' + this.value + ')';
